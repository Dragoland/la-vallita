...
{ label: 'Historia', href: '#historia', path: '/#historia' },
{ label: 'Legado', href: '#legado', path: '/#legado' },
{ label: 'Reconocimientos', href: '#reconocimientos', path: '/#reconocimientos' },
{ label: 'Catálogo', href: '#catalogo', path: '/#catalogo' },
{ label: 'Visítanos', href: '#visitanos', path: '/#visitanos' },
...
const Navbar: React.FC<Props> = ({ carrito }) => {
  ...
  const handleClick = (e: React.MouseEvent, link: typeof navLinks[0]) => {
    if (link.isRoute) {
      return; // Dejar que Link maneje esto
    }
    e.preventDefault();
    if (!isHome) {
      navigate(link.path);
