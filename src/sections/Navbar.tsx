...
const Navbar: React.FC<Props> = ({ carrito }) => {
  ...
  return (
    <header
      ...
      style={{
        background: scrolled 
          ? 'linear-gradient(180deg, var(--soil-dark), var(--soil))' 
          : 'linear-gradient(180deg, var(--soil-dark), var(--soil))',
        boxShadow: scrolled ? '0 4px 30px rgba(0,0,0,0.3)' : '0 2px 20px rgba(61, 41, 20, 0.25)',
    ...
    >
      <nav className="max-w-[1200px] mx-auto px-6 lg:px-8 flex items-center justify-between" style={{ height: '72px' }}>
        <Link to="/" className="flex items-center gap-3 group">
          ...
          <span className="font-serif font-bold text-2xl tracking-wide transition-colors duration-300" style={{ color: 'var(--white)' }}>
            La Vallita
          </span>
        </Link>

        <button
          className="lg:hidden border rounded px-3 py-1 text-lg transition-colors duration-300"
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 relative text-sm font-medium uppercase tracking-widest transition-colors duration-300 hover:text-[var(--wheat)]"
            style={{ color: 'rgba(255,255,255,0.85)' }}
          >
            <ShoppingCart size={18} />
            {carrito.totalItems > 0 && (
              <span className="absolute -top-2 -right-3 bg-[var(--wheat)] text-[var(--soil-dark)] text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center animate-bounce">
                {carrito.totalItems}
              </span>
            )}
          </button>
      ...
      <div className={`lg:hidden ${mobileOpen ? 'flex' : 'hidden'} flex-col gap-4 px-6 pb-6`} style={{ background: 'var(--soil-dark)' }}>
        ...
        <div className="flex items-center gap-4 pt-2 border-t" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
          <ThemeToggle />
          <button
