...
const Reconocimientos: React.FC = () => {
  ...
  return (
    <section id="reconocimientos" style={{ background: 'var(--bg-primary)', padding: '6rem 1.5rem' }}>
      <div className="max-w-[1200px] mx-auto">
        <SectionHeader label="Palmarés" title="Reconocimientos" subtitle="Lo que la tierra y el esfuerzo nos devolvieron" />
        <div ref={ref} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reconocimientos.map((rec, i) => (
            <div key={i} className="group relative rounded-2xl p-7 transition-all duration-300 hover:-translate-y-1.5 card-hover" style={{ background: 'var(--bg-card)', boxShadow: '0 4px 20px var(--shadow)', border: '1px solid var(--border-color)' }}>
              <div className="absolute top-0 left-0 right-0 h-1 rounded-t-2xl opacity-80" style={{ background: 'linear-gradient(90deg, var(--wheat), var(--leaf))' }} />
              <div className="flex items-center gap-2 mb-3">
                <Award size={16} style={{ color: 'var(--wheat)' }} />
                <span className="font-mono text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--wheat)' }}>{rec.year}</span>
              </div>
              <h4 className="font-serif text-xl font-bold mb-3" style={{ color: 'var(--soil-dark)' }}>{rec.title}</h4>
              <p className="text-sm leading-relaxed mb-4" style={{ color: 'var(--text-soft)' }}>{rec.desc}</p>
              <span className="inline-block text-xs font-semibold rounded px-3 py-1" style={{ background: 'rgba(74, 124, 46, 0.1)', color: 'var(--leaf)', letterSpacing: '0.5px' }}>{rec.tag}</span>
            </div>
          ))}
        </div>
      </div>
