...
const CarritoSidebar: React.FC<Props> = ({ carrito }) => {
    const { items, abierto, setAbierto, quitar, eliminar, vaciar, totalItems, totalCup, totalMlc, generarMensajeWhatsApp } = carrito;
    ...
    return (
        ...
        {items.length === 0 ? (
            ...
            items.map((item) => (
                ...
                <button onClick={() => {/* ya no tenemos acceso directo a agregar, lo manejamos en la card */}} className="p-1 hover:text-green-600 transition-colors opacity-50 cursor-not-allowed"><Plus size={14} /></button>
                </div>
